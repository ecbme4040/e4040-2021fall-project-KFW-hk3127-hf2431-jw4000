...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            bias1 = momentums[k] / (1 - beta1 ** t)
            bias2 = velocitys[k] / (1 - beta2 ** t)
            params[k] -= learning_rate * bias1/(bias2 ** (1/2) + eps)
            model.params = params
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            bias1 = momentums[k] / (1 - beta1 ** t)
            bias2 = velocitys[k] / (1 - beta2 ** t)
            params[k] -= learning_rate * (beta1 * bias1+(1-beta1) * grads[k] / (1- beta1 ** t)) / (bias2 ** (1/2) + eps)
            model.params = params
