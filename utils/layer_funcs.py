...
def conv2d_forward(x, w, b, pad, stride):
    ...
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width = ((width - filter_width + 2 * pad) // stride) + 1
    with_padding_h = np.concatenate((np.zeros((batch, height, pad, channels)),
                                     x,
                                     np.zeros((batch, height, pad, channels))), axis=2)
    X = np.concatenate((np.zeros((batch, pad, width + 2 * pad, channels)),
                        with_padding_h,
                        np.zeros((batch, pad, width + 2 * pad, channels))), axis=1)

    res = np.zeros((batch, new_height, new_width, num_of_filters))
            ...
            for k in range(new_width):
                for l in range(num_of_filters):
                    ...
                    for m in range(channels):
                        ...
                        tot += np.sum(X[i,
                                        j * stride : j * stride + filter_height,
                                        k * stride : k * stride + filter_width,
                                        m] * w[:, :, m, l])
                    res[i, j, k, l] = tot + bias
    return res
...
def avg_pool_forward(x, pool_size, stride):
    ...
    batch, height, width, num_of_filters = x.shape
    ...
    for i in range(batch):
        for j in range(new_height):
            for k in range(new_width):
                for l in range(num_of_filters):
                    res[i, j, k, l] = np.mean(x[i,
                                                j * stride : j * stride + pool_size,
                                                k * stride : k * stride + pool_size,
                                                l])
    return res
