...
class ImageGenerator(object):
    def __init__(self, x, y):
        ...
        num_of_samples, height, width, channels = x.shape
    ...
    def next_batch_gen(self, batch_size, shuffle=True):
        ...
        x_aug = self.x_aug
        y_aug = self.y_aug
        batch_N = self.N_aug // batch_size
        count = 0
        while True:
            if count < batch_N:
                count += 1
                yield (x_aug[(count-1) * batch_size : count * batch_size],
                       y_aug[(count-1) * batch_size : count * batch_size])
            else:
                if shuffle:
                    arr = np.arange(self.N_aug)
    ...
    def show(self, images):
        ...
        for i in range(16):
            plt.imshow(images[i].reshape((self.height, self.width)))
            plt.show()
    ...
    def translate(self, shift_height, shift_width):
        ...
        for i in range(self.N):
            for j in range(x.shape[3]):
                tmp = np.roll(x[i, :, :, j], shift=shift_height, axis=0)
                new_pic = np.roll(tmp, shift=shift_width, axis=1)
                x[i, :, :, j] = new_pic

        self.translated = (x, self.y)
    ...
    def rotate(self, angle=0.0):
        ...
        for i in range(self.N):
            for j in range(x.shape[3]):
                rotated = rotate(x[i, :, :, j], angle, reshape=False)
                x[i, :, :, j] = rotated

        self.rotated = (x, self.y)
    ...
    def add_noise(self, portion, amplitude):
        ...
        sample = np.random.choice(self.N, int(np.floor(portion * self.N) ), replace=False)
        for idx in sample:
            noise = np.random.normal(0, amplitude, shape)
            x[idx] = x[idx] + noise
