...
class MLP(object):
    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                dic = bn_params[i]
                gamma = params["bn_gamma_%s" % (i)]
                beta = params["bn_beta_%s" % (i)]
                cache_name = "batch_normalization_%s" % (i)
                x, cache[cache_name] = bn_forward(x, gamma, beta, dic, "train")
                bn_params[i] = dic
            ...
            if dropout_config["enabled"]:
                x, cache["dropout_%s" % (i)] = dropout_forward(x, dropout_config, "train")
        ...
        for i in range(num_layers - 1):
            ...
            if dropout_config["enabled"]:
                dx = dropout_backward(dx, cache["dropout_%s" % (j)])
            ...
            if use_bn:
                dx, dgamma, dbeta = bn_backward(dx, cache["batch_normalization_%s" % (j)])
                grads["bn_gamma_%s" % (j)] = dgamma
                grads["bn_beta_%s" % (j)] = dbeta
    ...
    def predict(self, X):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                beta = params["bn_beta_%s" % (i)]
                cache_name = "batch_normalization_%s" % (i)
                x, _ = bn_forward(x, gamma, beta, dic, "train")
                bn_params[i] = dic
            ...
            if dropout_config["enabled"]:
                x, _ = dropout_forward(x, dropout_config, "train")
