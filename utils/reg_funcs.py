...
def dropout_forward(x, dropout_config, mode):
    ...
    if mode == "train":
        ...
        N, D = x.shape
        U = np.random.rand(N, D) < keep_prob
        out = x * U
        cache = U
...
def bn_forward(x, gamma, beta, bn_params, mode):
    ...
    if mode == "train":
        ...
        var = np.var(x, axis = 0)
        x_sd = (x - mean) / np.sqrt((var + eps))
        out = gamma * x_sd + beta
        moving_mean = moving_mean * decay + (1 - decay) * moving_mean
        moving_var = moving_var * decay + (1 - decay) * moving_var

        # mean = tf.convert_to_tensor(mean, np.float32)
        # var = tf.convert_to_tensor(var, np.float32)
        # out = tf.convert_to_tensor(out, np.float32)
        # moving_mean = tf.convert_to_tensor(moving_mean, np.float32)
        # moving_var = tf.convert_to_tensor(moving_var, np.float32)
        ...
        x_sd = (x - mean[np.newaxis, :]) / np.sqrt((var + eps)[np.newaxis, :])
        out = gamma * x_sd + beta
